...
public class ReportRepositoryImpl implements ReportRepository {
    ...
    private final String selectReportWait = "SELECT REPORT_ID,REPORTER_ID,REPORTED_MEMBER_ID,ROOM_ID,REPORT_REASON,REPORT_STATUS,ADMIN_COMMENT,created_at,processed_at "
    +"from report_tb where report_status='WAIT'";
    private final String selectReportComplete = "SELECT REPORT_ID,REPORTER_ID,REPORTED_MEMBER_ID,ROOM_ID,REPORT_REASON,REPORT_STATUS,ADMIN_COMMENT,created_at,processed_at "
    +"from report_tb where report_status='COMPLETE'";                                    
    ...
    @Override
    public List<ReportVO> selectReportWait(){
        try {
            return jdbcTemplate.query(selectReportWait, (rs, rowNum) -> {
            ReportVO report = new ReportVO();
            report.setAdmin_comment(rs.getString("admin_comment"));
            report.setCreated_at(rs.getObject("created_at", LocalDateTime.class));
            report.setProcessed_at(rs.getObject("processed_at", LocalDateTime.class));
    ...
    @Override
    public List<ReportVO> selectReportComplete(){
        try {
            return jdbcTemplate.query(selectReportComplete, (rs, rowNum) -> {
            ReportVO report = new ReportVO();
            report.setAdmin_comment(rs.getString("admin_comment"));
            report.setCreated_at(rs.getObject("created_at", LocalDateTime.class));
            report.setProcessed_at(rs.getObject("processed_at", LocalDateTime.class));
